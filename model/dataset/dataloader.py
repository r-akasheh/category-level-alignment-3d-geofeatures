import torch
from model.dataset.HouseCat6d import HouseCat6dDataset, HouseCat6dTestset
from model.lib.utils import to_tensor
import pdb
...
def get_dataloader(config, split='train'):
    ...
    if split in ['train', 'val']:
        dataset = HouseCat6dDataset(config=config, split=split, data_augmentation=data_augmentation)
    else:
        dataset = HouseCat6dTestset(config=config, split=split, data_augmentation=data_augmentation)
